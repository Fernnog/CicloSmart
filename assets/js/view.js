/* --- ASSETS/JS/VIEW.JS --- */
/**
 * UI RENDERER (View Layer) - v1.5.0 (Refatoração: Duplo Indicador & Mobile Stats)
...
const ui = {
    ...
    render: () => {
        const executeRender = () => {
            ...
            let todayLoad = 0;
            let todayDoneLoad = 0; // NOVO: Guarda o tempo das tarefas finalizadas
            ...
            sorted.forEach(r => {
                if (r.date < todayStr && r.status !== 'DONE') {
                    ...
                    const studyTime = parseInt(r.time) || 0;
                    todayLoad += studyTime;

                    // NOVO: Soma apenas se a tarefa estiver feita
                    if (r.status === 'DONE') {
                        todayDoneLoad += studyTime;
                    }
                }
            });
            ...
            // ATUALIZADO: Passa agora o tempo realizado também
            ui.updateCapacityStats(todayLoad, todayDoneLoad);
            ui.updateStreak();
        };
    ...
    createCardHTML: (review) => {
        ...
        const tempIndicator = review.isTemporary 
            ? `<span class="inline-flex items-center gap-1 text-[10px] bg-amber-100 text-amber-700 border border-amber-300 px-2 py-0.5 rounded-full font-bold ml-2 cursor-help shadow-sm transition-all" title="${originText}. Voltará à origem amanhã se não for feito.">
                <i data-lucide="clock-4" class="w-3 h-3"></i> Extra
               </span>` 
            : '';
        ...
        // ----------------------------------------------------
        // LÓGICA DO BOTÃO DE REMANEJAMENTO (Puxar/Devolver)
        // ----------------------------------------------------
        let clockBtnHtml = '';
        if (review.date !== todayStr && review.status !== 'DONE') {
            clockBtnHtml = `
                <button onclick="app.moveToToday('${review.id}'); event.stopPropagation();" 
                        class="flex items-center justify-center p-1.5 rounded border border-slate-200 bg-slate-50 hover:bg-indigo-100 text-slate-400 hover:text-indigo-600 transition-colors ml-auto shadow-sm card-action-btn" 
                        title="Puxar para Hoje">
                    <i data-lucide="clock" class="w-4 h-4"></i>
                </button>
            `;
        } else if (review.isTemporary && review.date === todayStr && review.status !== 'DONE') {
            clockBtnHtml = `
                <button onclick="app.revertMove('${review.id}'); event.stopPropagation();" 
                        class="flex items-center justify-center p-1.5 rounded border border-amber-300 bg-amber-100 text-amber-700 hover:bg-amber-200 transition-colors ml-auto shadow-sm card-action-btn" 
                        title="Devolver à data original">
        ...
        return `
            <div id="card-${review.id}" ${draggableAttr} 
                ...
                <div class="flex items-center justify-between text-xs text-slate-400 pt-2 border-t border-slate-100 mt-2">
                    <div class="flex items-center gap-2">
                        <span class="flex items-center gap-1"><i data-lucide="clock" class="w-3 h-3"></i> ${review.time} min</span>
                        <span class="flex items-center gap-1"><i data-lucide="calendar" class="w-3 h-3"></i> ${formatDateDisplay(review.date)}</span>
                    </div>
                    ${clockBtnHtml}
                </div>
            </div>
    ...
    // REATORADO: Suporte a Duplo Indicador e Mobile
    updateCapacityStats: (todayMinutes, todayDoneMinutes = 0) => {
        const limit = store.capacity || 240;
        
        const percentage = Math.min((todayMinutes / limit) * 100, 100);
        const donePercentageLimit = Math.min((todayDoneMinutes / limit) * 100, 100);

        // Mapeamento de Elementos Desktop
        const bar = document.getElementById('capacity-bar');
        const text = document.getElementById('capacity-text');
        const doneBar = document.getElementById('capacity-done-bar');
        const doneText = document.getElementById('capacity-done-text');

        // Mapeamento de Elementos Mobile
        const barMobile = document.getElementById('capacity-bar-mobile');
        const textMobile = document.getElementById('capacity-text-mobile');
        const doneBarMobile = document.getElementById('capacity-done-bar-mobile');
        const doneTextMobile = document.getElementById('capacity-done-text-mobile');

        const updateUI = (b, t, db, dt, isMobile) => {
            if(b && t) {
                b.style.width = `${percentage}%`;
                t.innerHTML = isMobile ? `${todayMinutes}m` : `Total: <b>${todayMinutes}m</b> <span class="text-slate-300 mx-1">|</span> Meta: ${limit}m`;
                
                b.className = `h-full rounded-full transition-all duration-700 ease-out relative ${
                    percentage > 100 ? 'bg-slate-800' : percentage > 80 ? 'bg-red-600' : percentage > 60 ? 'bg-amber-500' : 'bg-indigo-600'
                }`;
            }
            
            if(db && dt) {
                db.style.width = `${donePercentageLimit}%`;
                dt.innerHTML = isMobile ? `${todayDoneMinutes}m` : `<b>${todayDoneMinutes}m</b> de ${todayMinutes}m`;
                
                // Celebração ao bater a meta do dia
                if(todayDoneMinutes >= todayMinutes && todayMinutes > 0) {
                    db.classList.add('bg-yellow-400');
                    db.classList.remove('bg-emerald-500');
                    dt.classList.add('text-yellow-600');
                    dt.classList.remove('text-emerald-700');
                    
                    if(!isMobile) dt.innerHTML = `<b>Metas 100% batidas! 🎉</b>`;

                    // FEEDBACK HÁPTICO (Vibração para Mobile)
                    if (navigator.vibrate) {
                        navigator.vibrate([100, 50, 100]);
                    }
                } else {
                    db.classList.add('bg-emerald-500');
                    db.classList.remove('bg-yellow-400');
                    dt.classList.add('text-emerald-700');
                    dt.classList.remove('text-yellow-600');
                }
            }
        };

        updateUI(bar, text, doneBar, doneText, false); // Atualiza Desktop
        updateUI(barMobile, textMobile, doneBarMobile, doneTextMobile, true); // Atualiza Mobile
    }
};
